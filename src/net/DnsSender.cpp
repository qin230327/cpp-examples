...
DnsSender::send(string domain, uint16_t localPort, string dnsIp,
    ...
    return UdpSender::send(*buildDnsBody(domain, queryRecur), localPort, dnsIp,
                           dnsPort);
}

std::shared_ptr<std::vector<std::uint8_t>> DnsSender::send(string domain,
                                                           string dnsIp,
                                                           uint16_t dnsPort,
                                                           bool queryRecur) {
    return UdpSender::send(*buildDnsBody(domain, queryRecur), dnsIp, dnsPort);
}
...
std::shared_ptr<std::vector<std::uint8_t>>
DnsSender::buildDnsBody(std::string domain, bool queryRecur) {
    ...
    std::shared_ptr<vector<std::uint8_t>> res =
        std::make_shared<std::vector<std::uint8_t>>();
    for (auto i = 0; i < len; i++) {
        res->push_back(bytePtr[i]);
    }
    return res;
}
} // namespace net